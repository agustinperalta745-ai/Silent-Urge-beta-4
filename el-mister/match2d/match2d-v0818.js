...
(function(){
  ...
  function roleLayout818(team,l){
    let roles=BASE_433.map(x=>x[0]);
    try{
      if(team===userSide818(l)&&typeof v06LiveLineup==='function'){
        const xi=v06LiveLineup();
        if(Array.isArray(xi)&&xi.length>=11)roles=xi.slice(0,11).map((p,i)=>p?.pos||BASE_433[i][0]);
      }
    }catch(e){}
    const slots=BASE_433.map((base,i)=>({role:roles[i]||base[0],x:base[1],y:base[2]}));
    const groups={ARQ:[],DEF:[],MID:[],ATT:[]};
    slots.forEach((s,i)=>{
      if(s.role==='ARQ')groups.ARQ.push(i);
      else if(['LI','LD','DFC'].includes(s.role))groups.DEF.push(i);
      else if(['MCD','MC','MCO'].includes(s.role))groups.MID.push(i);
      else groups.ATT.push(i);
    });
    const spread=(ids,x)=>ids.forEach((idx,k)=>{slots[idx].x=x;slots[idx].y=64*(k+1)/(ids.length+1)});
    spread(groups.ARQ,5);spread(groups.DEF,18);spread(groups.MID,40);spread(groups.ATT,66);
    if(team==='away')slots.forEach(s=>s.x=100-s.x);
    return slots;
  }

  function makePlayers818(team,l){
    return roleLayout818(team,l).map((s,i)=>({id:`${team}-${i}`,team,idx:i,role:s.role,x:s.x,y:s.y,baseX:s.x,baseY:s.y,tx:s.x,ty:s.y,speed:rnd818(.82,1.08)}));
  }
  ...
  function applySavedPlan818(l=S.live){
    ...
    const p=l.em818VisualPlan;
    if(p?.side&&p.plan)EM818.visual.plans[p.side]={...defaultPlan818(),...p.plan};
    const side=userSide818(l),plan=EM818.visual.plans[side];
  ...
  function updateTargets818(l=S.live){
    ...
    for(const p of v.players){
      const plan=v.plans[p.team],dir=p.team==='home'?1:-1;let x=p.baseX,y=p.baseY;
      x+=dir*plan.line*8;
      if(['LI','LD','LB','RB'].includes(p.role))x+=dir*plan.fullbacks*13;
      if(['EI','ED','LW','RW'].includes(p.role))x+=dir*plan.risk*5;
      if(['DC','ST'].includes(p.role))x+=dir*plan.direct*4;
      y=32+(y-32)*(1+plan.width*.18-plan.compact*.22);
      if(v.possession===p.team)x+=dir*(4.5+plan.tempo*2.4);
      else if(plan.press>0){x=lerp818(x,v.ball.x,clamp818(.09+plan.press*.12,0,.28));y=lerp818(y,v.ball.y,clamp818(.07+plan.press*.10,0,.24));}
      if(v.script?.type==='goal'&&v.script.team===p.team&&['DC','ST','EI','ED','LW','RW'].includes(p.role))x+=dir*10;
      p.tx=clamp818(x,3,97);p.ty=clamp818(y,3,61);
    }
  }
